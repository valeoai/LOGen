...
from logen.modules.PyTorchEMD.emd import earth_mover_distance
...
class RMSE():
    ...
    def update(self, gt, pt):
        error = (gt.cpu().detach().numpy() - pt.cpu().detach().numpy())
        squared = error**2
        mean = np.mean(squared)
        root = np.sqrt(mean)

        self.dists.append(root)
...
class JSD():
    ...
    def update(self, gt_np, pt_np):
        self.dists.append(jensenshannon(gt_np.cpu().detach().numpy(), pt_np.cpu().detach().numpy()).mean())
